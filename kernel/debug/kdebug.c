 ...
 * 根据eip 和n_type在.stab段中找到相应的stab项, 根据stab项的n_strx域,找到在.stabstr中的索引, 从该索引的字符串就是该项的
 * 名字, 或为文件名,或为函数名等.  如果n_type是N_LINE, 在.stab段中找到相应的行号
 *
 * */
/*
 * region_left , region_right 根据type返回代码的起始或终止地址, stabs 调试信息的起始地址, type类型, addr --> eip
 */
static void
